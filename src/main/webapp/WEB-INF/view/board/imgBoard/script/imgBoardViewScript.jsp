	...
	var vm = new Vue({
		...
		methods: {
			...
			goImgBoardUpdate: function(){
				var vm = this;
				window.location.href = "/goImgBoardUpdate?bdCode="+vm.bdCode+"&&bcCode="+vm.bcCode+"&&bcTitle="+vm.bcTitle+"&&bcRating="+vm.bcRating+"&&bcType="+vm.bcType+"&&pbcCode="+vm.pbcCode
										+"&&currentPage="+${currentPage}+"&&selected="+${selected}+"&&searchWrd="+'${searchWrd}';
			},
...
// 		var sto = spl[0] + spl[1]; 
	    ...
	    OpenWindow.document.write("<title>이미지 보기</title>");
		OpenWindow.document.write("<style>body{margin:0px;}</style><img src='"+url+"' width='"+win_width+"' onclick='self.close()' style='cursor:pointer;' title ='클릭하시면 창이 닫힙니다.'>");
	}
